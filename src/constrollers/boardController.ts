...
export const getPostsByBoardIdController = async (req: Request, res: Response): Promise<void> => {
    ...
    const { limit = 1, currentPage = 1 } = req.query;

    try {
        const posts = await getPostsByBoardId(Number(board_id), Number(limit), Number(currentPage));
        if(posts.length > 0) {
            res.status(StatusCodes.OK).json({ message: '게시물 목록 조회 성공' ,data : posts});
        }else{
